...
*Интервальное повторение • 8 режимов тренировки • Интерактивная грамматика • AI-генерация • Маркетплейс • Сообщество*

![Build Status](https://img.shields.io/badge/build-passing-brightgreen?style=flat-square)
![Tests](https://img.shields.io/badge/tests-246%20passed-success?style=flat-square)
...
- **Тёмная тема** с полной поддержкой Dark Mode на всех страницах
...
Полноценная система изучения грамматики с деревом навыков и интервальным повторением:

| Функция | Описание |
|---------|----------|
| 🌳 **Дерево навыков** | Визуальное дерево грамматических правил с прогрессом освоения |
| 📝 **7 типов упражнений** | MCQ, Transformation, Error Correction, Word Order, Translation, Matching, Dictation — генерируются AI |
| 🎧 **Диктанты с TTS** | Прослушивание предложения + ввод с клавиатуры, автовоспроизведение, кнопка медленного повтора |
| 🔗 **Сопоставление** | Соединение половин предложений с нумерацией пар и перемешиванием |
| 📚 **Порядок слов** | Составление предложения из перемешанных слов с подсветкой правильных/неправильных позиций |
| ⚙️ **Редактор правил** | CRUD для правил и упражнений всех 7 типов из веб-интерфейса |
| 🧠 **SM-2 для грамматики** | Интервальное повторение правил на основе результатов сессий |
| 📊 **Прогресс** | Статистика по каждому правилу: точность, количество сессий, уровень освоения |
| 🔔 **Due-ревью** | Автоматические напоминания о правилах, которые пора повторить |
| 🎯 **AI-валидация** | Двухэтапная проверка: генерация + валидация соответствия теме вторым LLM-вызовом |
| 🎯 **Интеграция** | Грамматика встроена в тренировку — кнопка перехода к due-правилам |
...
| 📖 **Грамматика** | Интерактивное дерево навыков с SM-2, 7 типов AI-упражнений (вкл. диктанты с TTS), due-ревью, редактор правил |
...
> **Ключевое отличие от упрощённого алгоритма:** Hard теперь снижает EaseFactor — сложные слова будут повторяться чаще. Интервалы ограничены потолком в 365 дней и растут адаптивно для каждого слова.

### SM-2 для грамматики

Грамматические правила также используют интервальное повторение. После каждой сессии упражнений результат маппится в качество ответа:

| Точность | Качество | Эффект |
|----------|----------|--------|
| ≥ 80% | Good | Интервал растёт |
| 50–79% | Hard | Интервал замедляется |
| < 50% | Again | Сброс интервала |
...
│   │       ├── MyDictionary     # Личный словарь с редактированием слов
│   │       ├── Rules            # Каталог правил
│   │       ├── RuleDetails      # Детали правила + упражнения
│   │       ├── MyRuleDetails    # Редактирование своего правила
│   │       ├── EditRule         # Редактор правила + CRUD упражнений (все 7 типов)
│   │       ├── Profile          # Публичный профиль пользователя
│   │       ├── MyContent        # Личный кабинет контента
│   │       ├── Training         # 8 режимов тренировки + Speed Round
│   │       ├── Grammar          # Дерево навыков грамматики
│   │       ├── GrammarPractice  # Сессия практики (7 типов: MCQ, Dictation, Matching...)
│   │       ├── Statistics       # Статистика, достижения, XP, грамматика
...
│   │                           # StatisticsApiService, ClassroomApiService,
│   │                           # AiApiService, GrammarApiService
...
│   │   ├── GrammarController   # Дерево навыков, практика, SM-2 грамматики
...
│   ├─ Program.cs              # Эндпоинты генерации + валидации
│   └─ Services/               # PromptTemplates (7 типов упражнений), AiProvider
├── 📦 LearningTrainerShared/   # Общая библиотека
│   ├── Models/                 # Entities, DTOs, Features
│   ├── Context/                # EF DbContext + миграции
│   └── Services/               # TokenService, SpacedRepetitionService (SM-2)
├── 🧪 LearningAPI.Tests/       # Unit-тесты (246 тестов)
│   ├── Controllers/            # 16 тестовых классов
...
| 🤖 **AI** | Ingat.AI микросервис (ASP.NET Core), LLM-интеграция, 7 типов упражнений, двухэтапная валидация |
...
| Method | Endpoint | Описание |
|--------|----------|----------|
| GET | `/api/grammar/skill-tree` | Дерево навыков с прогрессом |
| GET | `/api/grammar/due-reviews` | Правила, которые пора повторить |
| GET | `/api/grammar/{ruleId}/practice` | Получить упражнения для практики |
| POST | `/api/grammar/{ruleId}/submit-session` | Отправить результат сессии (SM-2) |
| POST | `/api/grammar/{ruleId}/generate-exercises` | AI-генерация упражнений (тип, сложность, кол-во) |
| GET | `/api/grammar/progress-summary` | Сводка прогресса по грамматике |
...
- `GrammarController` - дерево навыков, практика, submit-session, SM-2 грамматики
...
- `HealthController` - health checks, метрики
- `ImportController` / `ExportController` - JSON/CSV/Anki
...
| [LEARNING_IMPROVEMENTS.md](LEARNING_IMPROVEMENTS.md) | 📖 Анализ и план улучшений обучения |
...
- **Интерактивная грамматика:** дерево навыков, 7 типов AI-упражнений (MCQ, Transformation, Error Correction, Word Order, Translation, Matching, Dictation), SM-2, прогресс, редактор правил
- AI-генерация словарей через Ingat.AI
- XP-система, детальная статистика, достижения
- Настройки: тема, TTS громкость/скорость, дневная цель, режим тренировки, уведомления
